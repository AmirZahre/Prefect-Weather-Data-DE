...
def format_GET_hourly_api(raw_response_hourly_df):
    ...
    assert isinstance(raw_response_hourly_df,
                      pd.DataFrame), "raw_response_hourly_df should be a pandas DataFrame"

    response_hourly_df = raw_response_hourly_df.copy()

    # hashes the long/lat values
    city_id = response_hourly_df.apply(lambda x: gh.encode(
    ...
    # collapses df
    response_hourly_df = response_hourly_df.pivot_table(index=["city_id", "date"],
                                                        columns='parameter', values='value').reset_index()

    # remove index name
    response_hourly_df = response_hourly_df.rename_axis(
        None, axis=1)
    ...
    # create unique ID
    # hashes the long/lat values
    weather_id = response_hourly_df.apply(lambda x: str(int(time.mktime(
        x["date"].timetuple()))) + x["city_id"], axis=1)
