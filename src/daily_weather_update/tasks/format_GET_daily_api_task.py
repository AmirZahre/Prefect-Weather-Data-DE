...
def format_GET_daily_api(raw_response_daily_df):
    ...
    assert isinstance(
        raw_response_daily_df, pd.DataFrame
    ), "raw_response_daily_df should be a pandas DataFrame"

    response_daily_df = raw_response_daily_df.copy()  # create copy of raw df

    # hashes the long/lat values
    city_id = response_daily_df.apply(
    ...
    # collapses df
    response_daily_df = response_daily_df.pivot_table(
        index=["city_id", "date"], columns='parameter', values='value'
    ).reset_index()

    # remove index name
    response_daily_df = response_daily_df.rename_axis(None, axis=1)
    ...
    response_daily_df["sunrise_ux"] = [
        datetime.fromtimestamp(x) for x in response_daily_df["sunrise_ux"]
    ]
    response_daily_df["sunset_ux"] = [
        datetime.fromtimestamp(x) for x in response_daily_df["sunset_ux"]
    ]

    # create unique ID
    weather_id = response_daily_df.apply(
