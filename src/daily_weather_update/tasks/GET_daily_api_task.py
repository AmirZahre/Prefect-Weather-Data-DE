...
def GET_daily_api():
    ...
    period = f"{now}-06:00"  # 2023-01-01T08:05:00.000-06:00--
    daily_api_url = (
        f"https://api.meteomatics.com/"
        f"{period}/{params}/{locations}/json?model=mix"
    )

    # Making a get request
    response_daily = requests.get(
        daily_api_url, auth=HTTPBasicAuth(username, password)
    )

    raw_response_daily_json = response_daily.json()['data']

    # normalize df
    raw_response_daily_df = pd.json_normalize(
        raw_response_daily_json,
        ['coordinates', 'dates'],
        ['parameter', ['coordinates', 'lat'], ['coordinates', 'lon']],
    )

    # rename columns
    raw_response_daily_df.rename(
        columns={
